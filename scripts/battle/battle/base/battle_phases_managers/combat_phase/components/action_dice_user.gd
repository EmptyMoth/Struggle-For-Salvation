...
static func use_dice_in_one_side(character: Character, target: Character) -> void:
	if character.combat_model.get_next_dice().combat_model.is_used_in_one_side:
		...
		await character.combat_model.try_use_current_dice_in_one_side(target)
	else:
		prints(character, " ---- ")
		await character.combat_model.reserve_current_dice()
...
static func use_dice_in_clash(opponent_1: Character, opponent_2: Character) -> void:
	...
	BattleSignals.clash_continued.emit(opponent_1, opponent_2)
	var interaction: Callable = _get_action_dice_interaction(
			opponent_1.combat_model.get_next_dice().combat_model, opponent_2.combat_model.get_next_dice().combat_model)
...
static func _implement_reserve(opponent_1: Character, opponent_2: Character) -> void:
	...
	opponent_1.combat_model.reserve_current_dice()
	opponent_2.combat_model.reserve_current_dice()
...
static func _implement_respond(opponent_1: Character, opponent_2: Character) -> void:
	var first_is_responds: bool = opponent_1.combat_model.current_action_dice.combat_model.is_avoids_clash
	...
	await initiator.combat_model.try_use_current_dice_in_one_side(respondent)
	await respondent.combat_model.try_use_current_dice_in_one_side(initiator)
...
static func _implement_clash(opponent_1: Character, opponent_2: Character) -> void:
	...
	prints(opponent_1, " -><- ", opponent_2)
	if clash_result[opponent_1] == BattleEnums.ClashResult.WIN:
		opponent_2.combat_model.try_use_current_dice_in_clash(opponent_1, clash_result[opponent_2])
		await opponent_1.combat_model.try_use_current_dice_in_clash(opponent_2, clash_result[opponent_1])
	else:
		opponent_1.combat_model.try_use_current_dice_in_clash(opponent_2, clash_result[opponent_1])
		await opponent_2.combat_model.try_use_current_dice_in_clash(opponent_1, clash_result[opponent_2])
