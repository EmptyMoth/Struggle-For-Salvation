...
func animate_label() -> void:
	...
	tween.tween_property(self, "position:x", position.x - randi_range(-1, 1) * randi_range(50, 100), 0.75).set_ease(Tween.EASE_OUT)
	tween.tween_property(self, "position:y", position.y - randi_range(50, 100), 0.25).set_ease(Tween.EASE_OUT)
...
static func create_damage_label(damage_info: DamageInfo) -> DamageLabel:
	...
	label.text = str(damage_info.damage_received)
