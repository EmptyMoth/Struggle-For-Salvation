...
func add_speed_dice(count: int, is_ally: bool) -> void:
	for i in count:
		var speed_dice: AbstractSpeedDice = (
			preload("res://scenes/ui/battle/characters/speed_dice/speed_dice/ally_speed_dice.tscn").instantiate()
			if is_ally 
			else preload("res://scenes/ui/battle/characters/speed_dice/speed_dice/enemy_speed_dice.tscn").instantiate()
		)
