...
func _create_skill_selected_buttons(count: int) -> void:
	if count <= 0:
		return
	var skill_selected_button_scene: Resource = preload("res://scenes/ui/battle/popup_character_info/popup_character_additional_info/components/skill_selected_button.tscn")
	for i: int in count:
